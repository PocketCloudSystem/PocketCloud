...
public class TemplateEditData {
    ...
    public void applyTo(ITemplate template) {
        if (maxPlayerCount != null && maxPlayerCount < 0) throw new IllegalArgumentException("Max player count must be positive");
        if (minServerCount != null && minServerCount < 0) throw new IllegalArgumentException("Min server count must be positive");
        if (maxServerCount != null && maxServerCount < 0) throw new IllegalArgumentException("Max server count must be positive");
        if (startNewPercentage != null && (startNewPercentage < 0 || startNewPercentage > 1)) throw new IllegalArgumentException("Start new percentage must be between 0 and 1");
    ...
    public static TemplateEditData between(TemplateSettings oldSettings, TemplateSettings newSettings) {
        ...
        if (oldSettings.maintenance() != newSettings.maintenance()) templateEditData.maintenance = newSettings.maintenance();
        if (oldSettings.staticServers() != newSettings.staticServers()) templateEditData.staticServers = newSettings.staticServers();
        if (oldSettings.alwaysCopyToStaticServers() != newSettings.alwaysCopyToStaticServers()) templateEditData.alwaysCopyToStaticServers = newSettings.alwaysCopyToStaticServers();
        if (oldSettings.saveOnShutdown() != newSettings.saveOnShutdown()) templateEditData.saveOnShutdown = newSettings.saveOnShutdown();
        if (oldSettings.maxPlayerCount() != newSettings.maxPlayerCount()) templateEditData.maxPlayerCount = newSettings.maxPlayerCount();
        if (oldSettings.minServerCount() != newSettings.minServerCount()) templateEditData.minServerCount = newSettings.minServerCount();
        if (oldSettings.maxServerCount() != newSettings.maxServerCount()) templateEditData.maxServerCount = newSettings.maxServerCount();
        if (oldSettings.startNewPercentage() != newSettings.startNewPercentage()) templateEditData.startNewPercentage = newSettings.startNewPercentage();
