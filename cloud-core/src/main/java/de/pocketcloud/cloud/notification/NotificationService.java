...
public final class NotificationService {
    ...
    public boolean notify(NotificationType type, Map<String, Object> args, Map<Object, Object> extraArgs) {
        ...
        if (canSendWebhook(type)) {
            ...
            Webhook webhook = PocketCloud.instance().logSettingsConfig().discordWebhook().buildWebhook();
    ...
    public boolean canSendWebhook(NotificationType type) {
        return PocketCloud.instance().logSettingsConfig().discordWebhook().canSendWebhook(type);
    }

    public boolean canNotify(NotificationType type) {
        return PocketCloud.instance().logSettingsConfig().playerLogs().canNotify(type);
    }

    public boolean canLog(NotificationType type) {
        return PocketCloud.instance().logSettingsConfig().playerLogs().canLog(type);
    }
}