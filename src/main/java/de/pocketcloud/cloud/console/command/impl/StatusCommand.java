...
public final class StatusCommand extends Command {
    ...
    public boolean run(CommandSender sender, CommandContext ctx) {
        ...
        section(sender, "Traffic", lines -> {
            for (Map.Entry<String, Map<String, Long>> entry : TrafficMonitorManager.instance().allTimeTraffic().entrySet()) {
                String name = entry.getKey().substring(0, 1).toUpperCase() + entry.getKey().substring(1);
                String bytesIn = FormatUtils.bytes(entry.getValue().getOrDefault(TrafficMonitor.REGULAR_MODE_IN, 0L));
                String bytesOut = FormatUtils.bytes(entry.getValue().getOrDefault(TrafficMonitor.REGULAR_MODE_OUT, 0L));
                String avgBytesIn = FormatUtils.bytes(entry.getValue().getOrDefault(TrafficMonitor.REGULAR_MODE_IN + TrafficMonitor.SUFFIX_AVG, 0L));
                String avgBytesOut = FormatUtils.bytes(entry.getValue().getOrDefault(TrafficMonitor.REGULAR_MODE_OUT + TrafficMonitor.SUFFIX_AVG, 0L));
                lines.put(name + " All-Time Traffic", "§a" + bytesIn + " §8(§aIN§8) §8/ §c" + bytesOut + " §8(§cOUT§8)");
                lines.put(name + " Average Traffic", "§a" + avgBytesIn + "/s §8/ §c" + avgBytesOut + "/s");
            }
        });
