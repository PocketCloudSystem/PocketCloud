...
namespace pocketcloud\cloud\http\route\impl\v1\group;
...
final class AddTemplatesToGroupRoute extends ApiV1JsonPath {
    ...
    public function onHandle(Request $request, ResponseBuilder $builder, array $requestBody): void {
        ...
        foreach ($requestBody["templates"] as $template) {
            if (is_string($template)) {
                if (($template = TemplateManager::getInstance()->get($template)) !== null) {
                    $i++;
                    ServerGroupManager::getInstance()->addTemplate($group, $template, false);
                }
            }
        }
        ...
        $builder->body(["message" => "Added the templates to the group."]);
    }
