...
final class MonitorScreen extends IScreen {
    ...
    public function tick(int $currentTick): void {
        ...
        if ($currentTick % 20 === 0 || $this->firstCycle || $doNow) {
            ...
            $this->echo("§rMemory Usage: §b" . $formattedMemoryUsage . " " . $this->drawPctBars($memoryUsage, $memoryLimit, 10, $percentage) . " " . $this->formatPercentageWithColor($percentage) . $percentage . "%");
            $this->echo("§rCPU Usage: " . $this->drawPctBars($cpuUsage / ProcessUtils::getCpuCores(), 100, $percentage) . " " . $this->formatPercentageWithColor($percentage) . $percentage . "%");
